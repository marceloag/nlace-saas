...
export default function EditarCuenta() {
  ...
  useEffect(() => {
    const fetchAccount = async () => {
      ...
      if (account) {
        setTags(account.servicios);
        setAccountData(account);
        setIsLoading(false);
        setPromptAgente(account.prompt_agente);
      }
    };
    fetchAccount();
  }, [slug]);
  ...
  const handleSubmit = async (formData) => {
    ...
    const submit = async () => {
      try {
        ...
        formData.set('promptAgente', promptAgente);
        formData.set('slug', slug);
        const result = await updateAccount(formData);
        const nombreCuenta = accountData.nombre;

        if (result.success) {
          toast.success(
            `Se ha actualizado exitosamente la cuenta ${nombreCuenta} `
          );
        } else {
          toast.error(
          ...
          >
            <div className="space-y-2">
              <Input
                ...
                onChange={(e) =>
                  setAccountData({ ...accountData, nombre: e.target.value })
                }
              />
            </div>
            ...
            <div className="space-y-2">
              <Textarea
                ...
                onChange={(newdesc) =>
                  setAccountData({ ...accountData, descripcion: newdesc })
                }
              />
            </div>
            ...
            <div className="space-y-2" data-color-mode="light">
              <MDEditor
                value={promptAgente || ''}
                onChange={(val) => setPromptAgente(val)}
                preview="preview"
                height={700}
                theme="light"
                previewOptions={{
